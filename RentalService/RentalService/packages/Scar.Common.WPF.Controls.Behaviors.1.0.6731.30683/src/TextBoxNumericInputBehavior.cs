...
namespace Scar.Common.WPF.Controls.Behaviors
{
    public class TextBoxNumericInputBehavior : Behavior<TextBox>
    {
        ...
        public static readonly DependencyProperty OnlyPositiveProperty =
            DependencyProperty.Register(nameof(OnlyPositive), typeof(bool), typeof(TextBoxNumericInputBehavior),
        ...
        {
            if (string.IsNullOrEmpty(AssociatedObject.Text)) AssociatedObject.Text = Empty;
        }
        ...
        private string GetText([NotNull] string input)
        {
            ...
            if (txt.Text.Length < selectionStart) selectionStart = txt.Text.Length;

            var selectionLength = txt.SelectionLength;
            if (txt.Text.Length < selectionStart + selectionLength) selectionLength = txt.Text.Length - selectionStart;

            var realtext = txt.Text.Remove(selectionStart, selectionLength);

            var caretIndex = txt.CaretIndex;
            if (realtext.Length < caretIndex) caretIndex = realtext.Length;
            ...
            switch (InputMode)
            {
                ...
                case TextBoxInputMode.IntInput:
                    return int.TryParse(
                        ...
                        OnlyPositive
                            ? IntNumerStyles
                            : IntNumerStyles | NumberStyles.AllowLeadingSign,
                ...
                case TextBoxInputMode.DecimalInput:
                    var result = decimal.TryParse(
                        ...
                        OnlyPositive
                            ? DecimalNumberStyles
                            : DecimalNumberStyles | NumberStyles.AllowLeadingSign,
                        CultureInfo.CurrentCulture,
                        out _);
                    return result;
                default:
